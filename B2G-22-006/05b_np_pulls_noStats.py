...
for scenario in scenarios:
    ...
    command_step1 += ' -d ../../asymptoticlimits/workspace_' + year_to_run + '_fa' + fa + '_noStats_' + scenario + '.root' # workspace
    ...
    command_step1 += ' -n _noStats_' + scenario
    print(command_step1)
    # log_step1 = subprocess.run(['bash', '-c', command_step1], capture_output=True)
    # log_file1 = pulls_log_dir_name + 'fit_' + year_to_run + '_fa' + fa + '_noStats_' + scenario + '.log'
    ...
    command_step2 += ' fitDiagnostics_noStats_' + scenario + '.root'
    # command_step2 += ' --abs --all'
    command_step2 += ' -g pulls_noStats_' + scenario + '.root'
    print(command_step2)
    # log_step2 = subprocess.run(['bash', '-c', command_step2], capture_output=True)
    # log_file2 = pulls_log_dir_name + 'plotting_' + year_to_run + '_fa' + fa + '_noStats_' + scenario + '.log'
