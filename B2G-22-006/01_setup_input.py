...
def createCombineInput():
    ...
    print('creating input files for:')
    ...
    for entry in list_inputdirs:
        ...
        for fa in fas:
            ...
            datacard_name = 'datacard_' + year + '_' + channel + '_fa' + str(fa) + '.txt'
            with open(datacards_dir_name + datacard_name, 'w') as datacard:
                ...
                datacard.write('kmax *\n')
                datacard.write('shapes * * ' + filename_out + ' ' + var + '_$CHANNEL_$PROCESS ' + var + '_$CHANNEL_$PROCESS_$SYSTEMATIC\n')
                ...
                for rate in rates:
                    ...
                    for region in regions:
                        for process in processes:
                            if 'uncorrelated' in rate and year[:4] in rate: # uncorrelated lumi
                                datacard.write(pad(rates[rate], N4))
                            elif 'correlated' in rate and year[:4] in rate: # correlated lumi
                                datacard.write(pad(rates[rate][year[:4]], N4))
                    ...
                    for region in regions:
                        for process in processes:
                            if process in shapes[shape]:
                                ...
                                # if 'ttag' in shape:
                                #     datacard.write(pad('5.0', N4)) # inflate ttag uncertainties by factor 5
                                # else:
                                #     datacard.write(pad('1.0', N4))
                            else:
                                datacard.write(pad('-', N4))
