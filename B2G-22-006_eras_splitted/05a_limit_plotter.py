...
for scenario in scenarios:
    ...
    limit_recast_exp2p5 = np.loadtxt('../TOP-20-001/cts_mtt/asymptoticlimits/limit_expected2p5_' + year + '_' + scenario + '.txt', delimiter='\n')
    limit_recast_exp16p0 = np.loadtxt('../TOP-20-001/cts_mtt/asymptoticlimits/limit_expected16p0_' + year + '_' + scenario + '.txt', delimiter='\n')
    limit_recast_exp50p0 = np.loadtxt('../TOP-20-001/cts_mtt/asymptoticlimits/limit_expected50p0_' + year + '_' + scenario + '.txt', delimiter='\n')
    limit_recast_exp84p0 = np.loadtxt('../TOP-20-001/cts_mtt/asymptoticlimits/limit_expected84p0_' + year + '_' + scenario + '.txt', delimiter='\n')
    limit_recast_exp97p5 = np.loadtxt('../TOP-20-001/cts_mtt/asymptoticlimits/limit_expected97p5_' + year + '_' + scenario + '.txt', delimiter='\n')
    ...
    for limit in [limit_exp2p5, limit_exp16p0, limit_exp50p0, limit_exp84p0, limit_exp97p5]:
        limit *= np.square(fas)
    for limit in [limit_recast_exp2p5, limit_recast_exp16p0, limit_recast_exp50p0, limit_recast_exp84p0, limit_recast_exp97p5, limit_obs_recast]:
    ...
    # recast
    graph_recast_exp = root.TGraph(len(fas_recast), fas_recast, limit_recast_exp50p0)
    graph_recast_exp.SetLineColor(root.kBlue)
    graph_recast_exp.SetLineStyle(2)
    graph_recast_exp.SetLineWidth(2)

    graph_recast_obs = root.TGraph(len(fas_recast), fas_recast, limit_obs_recast)
    graph_recast_obs.SetLineColor(root.kBlue)
    graph_recast_obs.SetLineStyle(1)
    graph_recast_obs.SetLineWidth(2)

    graph_recast_2s = root.TGraphAsymmErrors(len(fas_recast), fas_recast, limit_recast_exp50p0, np.zeros(len(fas_recast)), np.zeros(len(fas_recast)), limit_recast_exp50p0-limit_recast_exp2p5, limit_recast_exp97p5-limit_recast_exp50p0)
    graph_recast_2s.SetFillColorAlpha(root.kBlue, 0.25)
    # graph_recast_2s.SetLineColorAlpha(root.kBlue, 0.25)
    graph_recast_2s.SetLineWidth(0)
    graph_recast_1s = root.TGraphAsymmErrors(len(fas_recast), fas_recast, limit_recast_exp50p0, np.zeros(len(fas_recast)), np.zeros(len(fas_recast)), limit_recast_exp50p0-limit_recast_exp16p0, limit_recast_exp84p0-limit_recast_exp50p0)
    graph_recast_1s.SetFillColorAlpha(root.kBlue, 0.5)
    # graph_recast_1s.SetLineColorAlpha(root.kBlue, 0.5)
    graph_recast_1s.SetLineWidth(0)
    ...
    legend.AddEntry(graph_recast_exp, 'Expected 95% CL upper limit (PRD 104, 092013)', 'l')
    legend.AddEntry(graph_recast_1s, 'Expected #pm 1#sigma (PRD 104, 092013)', 'fl')
    legend.AddEntry(graph_recast_2s, 'Expected #pm 2#sigma (PRD 104, 092013)', 'fl')
    legend.AddEntry(graph_recast_obs, 'Observed (PRD 104, 092013)', 'l')
    ...
    multigraph.Add(graph_recast_2s, 'SAME 3A')
    multigraph.Add(graph_recast_1s, 'SAME 3A')
    multigraph.Add(graph_recast_exp, 'SAME AL')
    multigraph.Add(graph_recast_obs, 'SAME AL')
    tlatex_cms.Draw()
    ...
    canvas.RedrawAxis()
    canvas.Update()
    canvas.SaveAs(dir_name + 'limits_' + year + '_' + scenario + '.pdf')
    # canvas.Delete()
